...
def calc_f1(y_true, y_pre, threshold=0.5):
    y_true = y_true.view(-1).cpu().detach().numpy().astype(np.int32) # 将矩阵拉直，实际相当于整体计算F1，即微平均
    y_pre = y_pre.view(-1).cpu().detach().numpy() > threshold
    return f1_score(y_true, y_pre)


def calc_acc(y_true, y_pre, threshold=0.5):
    y_true = y_true.view(-1).cpu().detach().numpy().astype(np.int32) # 将矩阵拉直，实际相当于整体计算ACC，即微平均
    y_pre = y_pre.view(-1).cpu().detach().numpy() > threshold
    return accuracy_score(y_true, y_pre)


def calc_sensitivity(y_true, y_pre, threshold=0.5):
    y_true = y_true.view(-1).cpu().detach().numpy().astype(np.int32) # 将矩阵拉直，实际相当于整体计算敏感度，即微平均
    y_pre = y_pre.view(-1).cpu().detach().numpy() > threshold
    return recall_score(y_true, y_pre)


def calc_precision(y_true, y_pre, threshold=0.5):
    y_true = y_true.view(-1).cpu().detach().numpy().astype(np.int32) # 将矩阵拉直，实际相当于整体计算精确率，即微平均
    y_pre = y_pre.view(-1).cpu().detach().numpy() > threshold
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            mask = torch.eq(labels, labels.T).float().to(self.device)  # [bsz, bsz] mask[i][j] = 0 or 1, means weather record[i] and record[j] belong to the same label class
        else:
            mask = mask.float().to(self.device)

        contrast_count = features.shape[1]
        contrast_feature = torch.cat(torch.unbind(features, dim=1), dim=0)  # [bsz+bsz, 128] i and i+bsz are two features from the same source record
        ...
        anchor_dot_contrast = torch.div(
            ...
            self.temperature)  # [bsz*2, bsz*2] anchor_dot_contrast[i][j] means z[i] inner product z[j] / temperature
                               # anchor_dot_contrast[i][j] = z[i]*z[j]/temperature
                               # only for contrast_mode == 'all'
        # for numerical stability
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)  # logits_max : [bsz+bsz, 1] logits_max[i][0] = max(anchor_dot_contrast[i][0..bsz*2-1]) # only for contrast_mode == 'all'
        logits = anchor_dot_contrast - logits_max.detach()  # logits : anchor_dot_contrast[i][j] - max(anchor_dot_contrast[i])

        # tile mask
        mask = mask.repeat(anchor_count, contrast_count)  # [bsz*2, bsz*2] now mask[i][j] (0, 1) refer to weather the labels of anchor_dot_contrast are equal # only for contrast_mode == 'all'
        # mask-out self-contrast cases
        logits_mask = torch.scatter(  # [bsz*2, bsz*2] eye matrix
            torch.ones_like(mask),
            1,
            torch.arange(batch_size * anchor_count).view(-1, 1).to(self.device),  # [bsz*2, 1] [i][0] = i
            0
        )
        mask = mask * logits_mask  # [bsz*2, bsz*2] this operatation makes mask[i][i] = 0

        # compute log_prob
        exp_logits = torch.exp(logits) * logits_mask  # [bsz*2, bsz*2] exp_logits = exp(logits) and this operatation makes exp_logits[i][i] = 0
        # if logits[i] logits[j] label is the same and i!= j, exp_logits[i][j] = exp(logits[i][j]) else 0
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))  # [bsz*2, bsz*2] log_prob[i][j] = logits[i][j] - sum(exp_logits(i))
...
class MLB_SupConLoss(nn.Module):
    ...
    def forward(self, features, labels, error_rate,
                weight_mode, formula_mode
                ):
        """Compute loss for model. If both `labels` and `mask` are None,
        it degenerates to SimCLR unsupervised loss:
        https://arxiv.org/pdf/2002.05709.pdf
        ...
        if formula_mode == 'direct_one':
            er_w_matrix = torch.matmul(er_w, er_w.T).to(self.device)  # [bsz, bsz] er_w_matrix[i][j] = er_w[i]*er_w[j]
...
class WeightedBCERDropLoss(nn.Module):
    ...
    def forward(self, outputs1, targets, outputs2=None,  train=False):
        ...
        if train:
            ...
            kl_loss1 = F.kl_div(F.logsigmoid(outputs1), torch.sigmoid(outputs2), reduction='batchmean')
            kl_loss2 = F.kl_div(F.logsigmoid(outputs2), torch.sigmoid(outputs1), reduction='batchmean')
...
class MultiLabelSCERDropLoss(nn.Module):
    ...
    def forward(self, outputs1, targets, outputs2=None,  train=False):
        ...
        if train:
            ce_loss = 0.5 * (ce_loss + self.MultilabelSoftmaxCrossEntropyLoss(outputs2, targets))
            kl_loss1 = F.kl_div(torch.log_softmax(outputs1, dim=-1), torch.softmax(outputs2, dim=-1), reduction='batchmean')
            kl_loss2 = F.kl_div(torch.log_softmax(outputs2, dim=-1), torch.softmax(outputs1, dim=-1), reduction='batchmean')
            kl_loss = 0.5 * (kl_loss1 + kl_loss2)
...
class HSICWeightedBCELoss(nn.Module):
    ...
    def forward(self, outputs, targets, global_feature, local_feature):
        ...
        bsz = outputs.shape[0]
        R = torch.eye(bsz).to(self.device) - (1 / bsz) * torch.ones(bsz, bsz).to(self.device)
...
class HSICWeightedBCERDropLoss(nn.Module):
    ...
    def forward(self, outputs1, targets, global_feature1, local_feature1, outputs2=None, global_feature2=None, local_feature2=None, train=False):
        hsic_ce_loss = self.HSICWeightedBCELoss(outputs1, targets, global_feature1, local_feature1)
        kl_loss = 0

        if train:
            ce_loss = 0.5 * (hsic_ce_loss + self.HSICWeightedBCELoss(outputs2, targets, global_feature2, local_feature2))
            kl_loss1 = F.kl_div(F.logsigmoid(outputs1), torch.sigmoid(outputs2), reduction='batchmean')
            kl_loss2 = F.kl_div(F.logsigmoid(outputs2), torch.sigmoid(outputs1), reduction='batchmean')
            kl_loss = 0.5 * (kl_loss1 + kl_loss2)
