...
def train_epoch(
    model: nn.Module, optimizer, criterion, train_dataloader, show_interval=10
):
    model.train()
    load_data_time = 0
    forward_time = 0
    ...
    f1_meter, loss_meter, it_count, acc_meter, sensitivity_meter, precision_meter = (
        0,
        0,
        0,
        0,
...
def train_procedure(args, model: nn.Module, model_save_dir: str) -> None:
    ...
    train_dataloader = DataLoader(
        ...
        shuffle=True,
        num_workers=6 if config.onserver else 1,
    )
    ...
    val_dataloader = DataLoader(
        ...
        batch_size=config.batch_size,
        shuffle=True,
        num_workers=4 if config.onserver else 1,
    )
    ...
    if (
        "loss_function" not in dir(config)
        or config.loss_function == "WeightedCrossEntropyLoss"
    ):
    ...
    if (
        "lr_scheduler" in dir(config)
        and config.lr_scheduler == "CosineAnnealingWarmRestarts"
    ):
        ...
        lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=config.T_0, eta_min=config.eta_min
        )
...
def train(args):
    ...
    if "MLGCN" in config.model_name:
        ...
        model = getattr(models, config.model_name)(
            ...
            t=config.MLGCN_t_threshold,
            gen_A_method=config.MLGCN_gen_A_method,
            device=device,
        )
    else:
...
def val(args):
    ...
    val_dataset = ECGDataset(data_path=config.train_data, train=False)
    val_dataloader = DataLoader(
        val_dataset, batch_size=config.batch_size, num_workers=4
    )
    ...
    for threshold in list_threhold:
        val_loss, val_f1, val_acc, val_sen, val_precision = val_epoch(
            model, criterion, val_dataloader
        )
        print(
            "val_loss:%0.3e val_f1:%.3f, val_acc:%.3f val_sen:%.3f, val_prec:%.3f\n"
            % (val_loss, val_f1, val_acc, val_sen, val_precision)
        )


def get_motified_transfered_model(
    model: nn.Module, transfer_model_name: str, train: bool = True
) -> None:
...
def trans_train(args):
    ...
    model_save_dir = "%s/%s_%s_%s_%s_%s" % (
        ...
        os.path.basename(
            os.path.dirname(os.path.abspath(config.pretrained_model_ckpt))
        ),
    )
    if args.ex:
        model_save_dir += args.ex
...
def test(args):
    ...
    sub_file = os.path.join(
        savedir,  # os.path.basename(os.path.dirname(os.path.abspath(args.ckpt)))+'_'+
        os.path.basename(args.ckpt)[:-4]
        + "_"
        + os.path.basename(config.test_label)[:-4]
        + config.test_expri_name
        + ".txt",
    )
    ...
    print(
        "Predict time without file operation for {} records: {}s".format(
            test_cnt, predicttime
        )
    )
    ...
    multiClassConfusionMatrix = np.zeros(
        shape=(len(idx2name), len(idx2name)), dtype=np.int32
    )
    ...
    with open(sub_file.split(".txt")[0] + ".csv", "w", encoding="GB2312") as output:
        output.write(
            "idx,arraythmia,quantity,TP,TN,FP,FN,specificity,sensitivity,precision,accuracy,F1,\n"
        )
        for idx in range(len(idx2name)):
            output.write(
                "{},{},{},{},{},{},{},{},{},{},{},{},\n".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", metavar="<command>", help="train or infer or trans_train"
    )
