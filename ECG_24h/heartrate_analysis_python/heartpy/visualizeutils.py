...
__all__ = ["plotter", "segment_plotter", "plot_poincare", "plot_breathing"]
...
def plotter(
    ...
    ax.scatter(
        np.asarray(peaklist) / fs,
        ybeat,
        color=colorpalette[1],
        label="BPM:%.2f" % (measures["bpm"]),
    )
    ax.scatter(
        rejectedpeaks / fs,
        rejectedpeaks_y,
...
def segment_plotter(
    ...
    # sanity check
    assert (
        0 < step < len(working_data["hr"])
    ), "step must be larger than zero and smaller than total number of segments"
    ...
    if end == None:
        ...
        assert end <= len(
            working_data["hr"]
        ), 'defined "end" endpoint is larger than number of segments'
...
def plot_poincare(
    working_data, measures, show=True, figsize=None, title="Poincare plot"
):  # pragma: no cover
    ...
    # plot scatter
    ax.scatter(
        x_plus, x_minus, color=colorpalette[0], alpha=0.75, label="peak-peak intervals"
    )
    ...
    ax.plot(
        identity_line, identity_line, color="black", alpha=0.5, label="identity line"
    )
    ...
    ax.plot(
        [np.mean(x_plus), np.mean(x_plus) + sd1_xrot],
        [np.mean(x_minus), np.mean(x_minus) + sd1_yrot],
        color=colorpalette[1],
        label="SD1",
    )
    ax.plot(
        [np.mean(x_plus), np.mean(x_plus) - sd2_xrot],
...
def plot_breathing(working_data, measures, show=True, figsize=None):  # pragma: no cover
    ...
    ax2.plot(
        working_data["breathing_frq"],
        working_data["breathing_psd"],
        label="spectrogram",
    )
    ax2.set_xlim(0, 1)
    ax2.set_xlabel("Hz")
