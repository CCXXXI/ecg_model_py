...
__all__ = ["filter_signal", "hampel_filter", "hampel_correcter", "smooth_signal"]
...
def filter_signal(
    data, cutoff, sample_rate, order=2, filtertype="lowpass", return_top=False
):
    ...
    if filtertype.lower() == "lowpass":
        ...
        assert (
            type(cutoff) == tuple or list or np.array
        ), "if bandpass filter is specified, \
cutoff needs to be array or tuple specifying lower and upper bound: [lower, upper]."
        ...
        raise ValueError(
            "filtertype: %s is unknown, available are: \
