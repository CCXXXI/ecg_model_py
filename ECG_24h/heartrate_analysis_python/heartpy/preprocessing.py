...
def scale_sections(data, sample_rate, windowsize=2.5, lower=0, upper=1024):
    ...
    warnings.warn(
        "scale_sections function is deprecated and will be removed in a future release",
    ...
    for i in range(0, len(clipping_edges)):
        if i == 0:  # if first clipping segment
            clipping_segments.append(
                (clip_binary[0][0], clip_binary[0][clipping_edges[0]])
            )
        elif i == len(clipping_edges) - 1:
            # append last entry
            clipping_segments.append(
                (clip_binary[0][clipping_edges[i] + 1], clip_binary[0][-1])
            )
        else:
            clipping_segments.append(
                (
                    clip_binary[0][clipping_edges[i - 1] + 1],
...
def interpolate_clipping(data, sample_rate, threshold=1020):
    ...
    for segment in clipping_segments:
        if segment[0] < num_datapoints:
            ...
            interpdata_x = np.concatenate(
                (
                    [x for x in range(segment[0] - num_datapoints, segment[0])],
            ...
            try:
                ...
                data[
                    segment[0] - num_datapoints : segment[1] + num_datapoints
                ] = interp_data
            except:
...
def enhance_ecg_peaks(
    hrdata, sample_rate, iterations=4, aggregation="mean", notch_filter=True
):
    """enhances ECG peaks
...
def generate_ecg_templates(
    sample_rate,
    widths=[50, 60, 70, 80, 100],
    presets=[[0, 2, 2.5, 3, 3.5, 5], [0, 1, 1.5, 2, 2.5, 3], [0, 3, 3.5, 4, 4.5, 6]],
