...
def U_net_peak(data,input_fs,band_Hz = 0.5,del_drift= True , target_fs = 240,model = None,gpu=False,device = 'cpu'):
    #提取U-net波群信息
...
def R_Detection_U_net(data,N):
    ...
    for i in range(lenx):
        x[i] = (x[max(i-2,0)]+x[max(i-1,0)]+x[i]+x[min(i+1,lenx-1)]+x[min(i+2,lenx-1)])/5
