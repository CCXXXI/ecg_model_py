...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=55, att_type=None, smapool=None):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.smapool = None
        if smapool:
            self.smapool = nn.AvgPool1d(kernel_size=smapool, stride=1, padding=smapool//2)
...
def resnet34_cbam_sma5(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3], att_type='CBAM', smapool=5, **kwargs)
    return model
