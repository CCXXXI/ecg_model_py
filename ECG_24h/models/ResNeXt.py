...
class ECGResNeXt32(nn.Module):
    ...
    def __init__(self, cardinality, nlabels, base_width, widen_factor, kernal_size):
        ...
        # print(self.stages)
        self.stages = [
            64 * (self.widen_factor**i) for i in range(self.stages_num + 1)
        ]
        # print(self.stages)
        ...
        self.conv_1_k = nn.Conv1d(
            leads_num, 64, kernal_size, 1, kernal_size // 2, bias=False
        )
        self.bn_1 = nn.BatchNorm1d(64)
...
def ResNeXt29_conv3_cd8(num_classes: int) -> nn.Module:
    model = ECGResNeXt(
        cardinality=8, depth=29, nlabels=num_classes, base_width=64, widen_factor=4
    )
    return model
...
def ResNeXt32_conv7_cd8(num_classes: int) -> nn.Module:
    model = ECGResNeXt32(
        cardinality=8, nlabels=num_classes, base_width=64, widen_factor=2, kernal_size=7
    )
    return model
