...
class Config:
    ...
    # 本地的数据根目录
    root = 'D://Data//Heartbeat'
    if onserver:
        root = '/home/ECG_AI/xintiao/data/heartbeat'
    ...
    alldata_label = os.path.join(root, 'all_label_v1.txt')  # 整个数据集的标签，包括测试集验证集训练集，在划分测试集时使用
    ...
    dir_24h = os.path.join(root, '24hdata')# 24小时数据路径
    beats_24h = os.path.join(root, '24hbeat')# 24小时心拍列表，由24小时数据路径内提取出
    R_24h = os.path.join(root, '24hRpeak')# 24小时R波列表
    mybeats_24h = os.path.join(root,'24hMybeat')

    test_expri_name = 'no_data_standard'
    experiment_name = 'v1_lr1e-3_st16_32_64_warmup1e-5-8_bsz1280.005mv_nounittransfrom'  # 实验名称，会追加到保存模型的文件夹名
